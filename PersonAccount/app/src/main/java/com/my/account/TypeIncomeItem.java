...
public class TypeIncomeItem extends Activity implements View.OnClickListener {
    //消息码
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //加载布局
        setContentView(R.layout.manger_income_type_item);
        init();//初始化
        //获取Bundle
...
//    }
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            int save_success=save_income_type(type_id,pngId,is_default,type_name);
    ...
    /**
     * 保存
     * @param type_id 收入类型id
     * @param pngId   图片id
     * @param is_default 是否是系统默认类别
     * @return
     * @throws Exception
     */
    public int save_income_type(int type_id,int pngId,int is_default,String name) throws Exception{
        String op="MangerTypeServlet?method=update_income_type"
                +"&&type_id="+type_id
                +"&&pngId="+pngId
     ...
     * @throws Exception
     */
    public int del_income_type(int type_id,int permissions) throws Exception{
        String op="MangerTypeServlet?method=del_income_type"
                + "&&type_id="+type_id
    ...
    private int getImageIdByName(String name) throws Exception  {
        ...
        switch (name){
            case "工资薪水":
              field=R.drawable.class.getField("income_pay_salary");
              id=Integer.parseInt(field.get(null).toString());
                break;
            case "利息":
              field=R.drawable.class.getField("income_interest");
              id=Integer.parseInt(field.get(null).toString());
                break;
            case "兼职外快":
                field=R.drawable.class.getField("income_part_time");
                id=Integer.parseInt(field.get(null).toString());
                break;
            case "营业收入":

                field=R.drawable.class.getField("income_business");
                id=Integer.parseInt(field.get(null).toString());
                break;
            case "红包":
                field=R.drawable.class.getField("income_red_envelope");
                id=Integer.parseInt(field.get(null).toString());
                break;
            case "销售额":
                field=R.drawable.class.getField("income_sales");
                id=Integer.parseInt(field.get(null).toString());
                break;
            case "退款返款":
                field=R.drawable.class.getField("income_reimburse");
                id=Integer.parseInt(field.get(null).toString());
                break;
            case "报销款":
                field=R.drawable.class.getField("income_dispatch");
                id=Integer.parseInt(field.get(null).toString());
                break;
        }
        return id;
                ...
                .setPositiveButton("是", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        new Thread(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    int del_success=del_income_type(type_id,permissions);
                                    if(del_success==1){
                                        Message message=new Message();//消息
